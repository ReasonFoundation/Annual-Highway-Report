      ...
      tabPanel("Methodology",
               fluidRow(
                 column(10, offset = 1,
                        ...
                        p("SHA Ratio = Total lane-miles / State Highway Agency miles."),
                        p("Ranked directly on ratio (higher = more local control).")
                 )
               )
      )
    )
...
server <- function(input, output, session) {
  ...
  table_data_r <- reactive({
    ...
    if (cat_val %in% category_choices[3:length(category_choices)]) {
      ...
      df <- mileage %>%
        filter(year == year_val) %>%
        select(State = state, Year = year, `SHA Miles` = SHA_miles,
               `Total Lane Miles` = state_tot_lane_miles, `SHA Ratio` = SHA_ratio)
    }
