...
#NOTE: INRIX and ACS commuter data are collected using a separate project in this repo: https://github.com/thuy2020/inrix_scraping
#INRIX data 

inrix <- read_csv("AHR 30th/data/inrix_delay_hour_2024_us.csv") 
...
commuter_data <- read_csv("AHR 30th/data/acs_2024_s0802_metros.csv")
...
inrix_clean <- inrix %>% 
  ...
  rename(hours_lost_in_congestion = `2024_delay`,
         state = state_abbr) 
...
process_hm74 <- function(sheet_name) {
  read_excel("AHR 30th/data/hm74.xlsx", sheet = sheet_name) %>%
    slice(-(1:8)) %>% 
    select(1:27) %>% 
    rename(area = 1,
           state = 2, 
...
excel_sheets("AHR 30th/data/hm74.xlsx")
sheets <- 2:9
