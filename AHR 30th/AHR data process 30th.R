...
#Congestion data 
source("AHR 30th/Congestion data process.R")
...
HM_10 <- read_excel("AHR 30th/data/hm10.xlsx", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 2, 5, 8, 11) %>% 
  slice(-(1:6)) %>% 
  rename(state = 1,
...
# sc_HM64_rural_interstate <- read_excel("AHR 30th/data/hm64_year2022.xls", sheet = "A") %>% 
#   remove_empty() %>% 
#   select(1, 8:11) %>% 
#   rename(state = 1,
...
# sc_HM64_urban_interstate <- read_excel("AHR 30th/data/hm64_year2022.xls", sheet = "C") %>% 
#   remove_empty() %>% 
#   select(1, 8:11) %>% 
#   rename(state = 1,
...
HM64_rural_interstate <- read_excel("AHR 30th/data/hm64.xlsx", sheet = "A") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
         rural_interstate_171_194 = 2,
...
HM64_urban_interstate <- read_excel("AHR 30th/data/hm64.xlsx", sheet = "C") %>% 
  remove_empty() %>% 
  select(1, 8:11) %>% 
  rename(state = 1,
         urban_interstate_171_194 = 2,
...
FI_20 <- read_excel("AHR 30th/data/fi20.xlsx", sheet = "A", skip = 13) %>% 
  rename(state = 1) %>% 
...
all_rankings_export <- all_rankings %>%
  left_join(
    AHR_data %>%
      select(state, state_avg_congestion_hours),
    by = "state"
  )
...
data_list <- list(
  ...
  "All Rankings" = all_rankings_export,
